...
| F2-3 智能电台 | ✅ 完成（M2）+ 多源化演进（2026-09-11） | 1.5 天 | | 首期仅 NAS 曲库；天气电台 Tab 入口降级为 NowPlaying 单入口。演进：推荐池扩展为 NAS+本地+Meting 多源，入口迁至首页独立区块（天气卡上方），NowPlaying 入口移除，与播放状态解耦（详见 technical-overview §10.119）。手测反馈迭代：交互改随心听式浏览列表（generateOnly 只生成不播放 + 点卡片播歌 + 换一批，详见 §10.120） |

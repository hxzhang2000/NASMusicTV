...
**版本号**：versionCode=6, versionName="2.3.0-dev"

**重大修复**：

| 修复项 | 文件 | 描述 |
|--------|------|------|
| MP3 流 Seek | `PlaybackService.kt`, `PlayerManager.kt` | 启用 `FLAG_ENABLE_INDEX_SEEKING` + `seekPending` 保护 |
| 进度条 OK 键 | `PlayerControls.kt` | 移除 Surface onClick，OK 键不触发 seek |
| 播放按钮闪烁 | `PlayerManager.kt` | `onIsPlayingChanged` 在 seekPending 期间跳过 |
| 专辑/艺术家详情页 | `AppRoot.kt`, `MainViewModel.kt` | 响应式 StateFlow 按需加载歌曲列表 |
| 艺术家封面 | `JellyfinAdapter.kt`, `LibraryScreen.kt`, `ArtistDetailScreen.kt` | `getArtists()` 添加 `Fields=ImageTags`，UI 添加 `AsyncImage` |
| 编码修复 | `EncodingUtils.kt` | U+FFFD 检测 + GBK 回退 |
| 清空队列歌词 | `MainViewModel.kt` | `clearQueue()` 同时清除 `_currentLyrics` |
| 队列删除按钮 | `QueueScreen.kt` | 添加 ✕ 按钮，焦点导航修复 |
| 歌词来源 | `LyricsManager.kt` | 后端/网络歌词同时获取，标签均亮起 |

**UI 改进**：
- "歌唱家"改名为"艺术家"
- 队列删除/移动按钮移至 `FocusableSurface` 外部（可聚焦）
- 歌词来源标签"后端"/"网络"同时亮起

**回归测试**：24 项通过 / 1 项缺陷（暗色主题无效）/ 6 项跳过

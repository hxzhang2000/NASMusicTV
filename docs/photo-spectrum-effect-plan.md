...
### ✅ 实际投入回填（T13.1，2026-09-23 交付后实测）

| 项 | 预估 | 实际 | 说明 |
|---|---|---|---|
| 新增 Kotlin 源文件 | — | **32 个** | §14.1 清单逐行核对（含实现期补的 `PhotoWallAccessPolicy` 等） |
| 主代码行数 | 3920–4520 | **≈ 6,950 行** | 超出主因：① 28 个 P1 转场比粗估细（~1,150 行）；② `PhotoWallController` 728 行（预估 250，含拔盘 / 解码超时 / 计数流）；③ 设置分区 619 行（预估 450，含运行时状态与置灰）；④ KDoc 占比高（项目惯例，非注释膨胀） |
| 单测行数 | ~750 | **≈ 4,200 行**（17 个测试类 / +180 例） | 每个门禁都带负向自证 + Robolectric 真跑帧循环 |
| 分期执行 | P0 → P1 → P2 | **阶段 1–12**（12 个提交） | ⚠️ 两个排期调整：**人脸检测从 P2 提前**（随实现窗口一次做完，避免二次上下文）；**P2 的 33 种转场明确不在本期**（§15.3 阶段 12 偏差 1 声明的范围） |
| 数据库 | `LocalMusicDatabase` 升版 | **独立建库** `photo_face.db` v1 | 实现期决策（§15.3 阶段 11 偏差）：人脸表与音乐库生命周期无关，不升 v3 → v4 |
| 模型资产 | yunet 2023mar 337 KB | **2026may 224 KB**（动态输入） | 见 §15.3 阶段 11 偏差 1 |

> ⚠️ 交付口径：**代码全部完成、门禁全绿（1143 例 / 0 失败，lint 0 Error）**；
> §14.6 的真机验收项（电视 8 条 + 手机 13 条）按项目约定**由用户以 release 包复验**后关闭。
...
> 📏 **实际行数（T13.1 回填）**：主代码合计 **≈ 6,950 行**（下表「预估」为规划值，未逐行改写——
> 大头偏差见 §十二 实际投入回填：`PhotoWallController` 728 / 设置分区 619 / P1 转场 ~1,150）。

...
### 4.22 候选效果可行性分析（v6.1 新增）

以下三套效果为 v2.7.0 测试后用户提议的新增候选，按可行性与实现成本逐一评估。

---

#### 4.22.1 E21 `PRISM` 棱镜 ★★

**概述**：三棱镜折射效果——白光穿过棱镜后色散为彩虹色光谱带，频谱柱以彩色渐变扇形从中心向外扩散，带辉光与折射扭曲感。

| 维度 | 评估 |
|------|------|
| **可行性** | ✅ **高** |
| **技术复用** | 90% 基于现有能力：`drawArc` + 渐变 + `BlendMode.Plus` + 频谱驱动 |
| **核心实现** | ① 中心圆盘（白光）→ ② 多个扇形 `drawArc` 按频谱高度展开，每段 hue 渐变（黄→绿→蓝→紫）→ ③ 外层辉光环 → ④ `bass` 驱动整体缩放 + 轻微扭曲偏移 |
| **映射** | `spectrum[i]` → 对应扇区弧长；`bass` → 中心圆盘半径 + 整体缩放；`pulse` → 辉光强度；`treble` → 色散程度（扇区间距） |
| **风险** | 无硬风险。需要注意扇区数量与 barCount 对齐，避免越界 |
| **预估成本** | 约 2–3ms（64 个 arc + 1 个中心圆 + 1 个辉光环） |
| **实现难度** | ★★☆☆☆（低-中） |
| **档位** | BASIC 级，三档全可用（LOW 降为 32 扇区） |
| **预计工期** | 0.5 天 |

**设计要点**：
- 扇区数 = `barCount`，每扇区角度 = 360° / barCount
- 每扇区使用 `drawArc` 绘制，起始半径 = `r0`，扫过角度 = 扇区角 × (0.3 + v × 0.7)
- 颜色：每扇区 hue 沿频谱方向渐变（60°→195°，黄→绿→蓝），外端更亮
- 中心白色圆盘模拟"入射光"，`bass` 强时扩大
- 叠加 `BlendMode.Plus` 让重叠处产生色散交叠感

---

#### 4.22.2 E22 `AURORA` 极光 ★★★

**概述**：北极光风格——多条柔和的光幕带在屏幕上半部分缓慢流动、扭曲，颜色在绿/蓝/紫之间渐变，低频驱动光幕起伏，能量驱动亮度。

| 维度 | 评估 |
|------|------|
| **可行性** | ✅ **高** |
| **技术复用** | 70% 基于现有能力：`Path` + `cubicTo` 平滑曲线 + 多层半透明填充 + 频谱驱动。与 E07 频率山峦 有相似的曲线+填充技法 |
| **核心实现** | ① 3–5 条极光带，每条由 `Path.cubicTo` 定义的平滑贝塞尔曲线构成 → ② 每条带填充半透明渐变（上淡下浓） → ③ 极光带位置/形状随时间缓慢漂移 + 频谱扰动 → ④ `BlendMode.Plus` 叠加发光 |
| **映射** | `spectrum[i]` → 极光带局部高度扰动；`bass` → 整体起伏幅度；`mid` → 流动速度；`sectionEnergy` → 极光带数量（2–5 条）与整体亮度 |
| **风险** | 无硬风险。曲线控制点较多，调参需要时间；需保证"柔和感"，不能变成尖锐波形 |
| **预估成本** | 约 3–5ms（3–5 条 `drawPath` 填充 + 曲线控制点计算） |
| **实现难度** | ★★★☆☆（中） |
| **档位** | ADV 级（HIGH/MEDIUM 可用，LOW 禁用） |
| **预计工期** | 1–1.5 天 |

**设计要点**：
- 极光带分布在屏幕上 2/3 区域，底部淡出到黑
- 每条带 2 层：底层宽淡（alpha 0.15）+ 顶层窄亮（alpha 0.4），模拟"光柱"感
- 控制点使用正弦叠加扰动：`y = baseY + sin(x*0.01 + t*0.001)*bass*40 + sin(x*0.03 + t*0.002)*mid*20`
- 颜色：主绿(120°) + 次蓝(200°) + 点缀紫(280°)，沿流动方向缓慢色相偏移
- 节拍时整体亮度 +30%，模拟"极光爆发"
- 顶部可加少量星点背景（复用 CONSTELLATION 的星点池简化版），增强夜空感

---

#### 4.22.3 E23 `LYRICS_DOT_MATRIX` 歌词点阵 ★★★

**概述**：屏幕正中央显示由发光粒子组成的当前歌词文字，逐字点亮（唱到哪个字哪个字的粒子最亮），
唱完一句后粒子向外扩散消失，下一句粒子从屏幕各处飞来重新凝聚成文字。
与 E19 粒子文字的区别：E19 是歌名常驻 + beat 炸散，E23 是**逐句/逐字歌词** + 凝聚/消散过渡 + 逐字点亮。

| 维度 | 评估 |
|------|------|
| **可行性** | ✅ **高**（已有完整逐字歌词基础设施可复用） |
| **技术复用** | **80%** 基于现有能力：E19 粒子文字（Bitmap 采样 + `ParticlePool` + `updateAttract`）+ **已有逐字歌词系统** |
| **已有基础设施（直接复用，无需重写）** | ① `LrcParser` 已支持增强 LRC 逐字时间戳解析（`<mm:ss.xx>字` 格式）→ `WordTimestamp(word, startMs)`；② `LyricsLine.wordTimestamps` 数据结构已存在；③ `LyricsManager` 已提供逐字歌词加载；④ `MainViewModel.lyricsHighlightMode` 已有 `WORD_BY_WORD` 模式；⑤ `LyricsManager.getCurrentLineIndex()` 二分查找可扩展为逐字查找 |
| **核心实现** | ① 监听当前歌词行变化 → ② 新行文字 `Bitmap.getPixel()` 采样为目标点阵（逐字分组记录） → ③ 粒子从当前位置吸向新目标点（凝聚动画，300–500ms）→ ④ 行持续期间：已唱过的字粒子偏暗、当前字最亮、未唱的字偏暗 → ⑤ 下一行到来时，旧粒子炸散 + 新粒子凝聚（两行交叉过渡） |
| **映射** | `spectrum[i]` → 粒子抖动幅度；`treble` → 粒子闪烁频率；`bass` → 粒子尺寸脉冲；`beat` → 粒子短暂外扩后回弹；**逐字时间戳 → 逐字点亮进度** |
| **依赖** | `LyricsManager` / `MainViewModel` 提供当前行文字、逐字时间戳、播放进度 → 渲染器通过 `RenderContext.caption` 扩展或新增 `lyricsState` 字段传入 |
| **风险** | ① 无逐字歌词时退化？→ 回落为逐行模式（整行同时亮，无逐字效果）；② 无任何歌词时退化？→ 显示歌名（同 E19）；③ 行切换频繁（快歌 rap）时粒子来不及凝聚 → 最短凝聚时间 300ms，短于 300ms 的行跳过过渡直接显示；④ 中文/英文文字宽度差异大 → 采样时按文字长度自适应缩放，目标点数量固定（按档位 200/350） |
| **预估成本** | 约 4–6ms（粒子更新 + 绘制，与 E19 同量级）+ 切行时一次性采样开销（5–15ms，异步执行） |
| **实现难度** | ★★★☆☆（中）——逐字歌词基础设施已存在，主要工作量在粒子逐字点亮逻辑 |
| **档位** | ULTRA 级（仅 HIGH 可用，MEDIUM/LOW 跳过并提示） |
| **预计工期** | **1.5–2 天**（原估 2–2.5 天，因复用逐字基础设施减少约 0.5–1 天） |

**设计要点**：
- 粒子池复用 `ParticlePool`，容量同 E19（HIGH 350）
- 目标点采样：`Canvas.drawText()` → `Bitmap.getPixel()` 扫描（同 E19 降级方案）
- **逐字分组**：采样时记录每个字对应的粒子索引范围，用于逐字点亮
- 逐字点亮逻辑：
  - 已有 `wordTimestamps` 时：根据 `progressMs` 找到当前字，当前字粒子 100% 亮度 + 白色偏移，已唱过的字 60% 亮度，未唱的字 30% 亮度
  - 无逐字时间戳时：退化为整行统一亮度（仍有凝聚/消散动画，但无逐字效果）
- 状态机：`COALESCING`（凝聚中）→ `HOLDING`（保持+逐字点亮+微抖）→ `SCATTERING`（消散中）
- 双行过渡：旧行进入 SCATTERING 的同时新行开始 COALESCING，重叠 200ms
- 无歌词/纯音乐时回落：显示当前歌曲名（同 E19），`beat` 时炸散
- 字体：等宽字体（MONOSPACE），字号自适应（保证整行不超过屏幕宽度 80%）
- 颜色：用户指定色系（亮黄/亮绿/亮蓝/白），当前字用白色高亮
- **`RenderContext` 扩展**：新增 `currentLyricLine: String?`、`lyricProgress: Float`、`wordTimestamps: List<WordTimestamp>?` 三个字段（或打包为 `LyricState`），由 `VisualizerStage` 每帧更新
...
| 优先级 | 效果 | 理由 |
|--------|------|------|
| **P1（推荐优先做）** | **E21 棱镜** | 实现成本最低（0.5 天），视觉效果鲜明，与现有圆形频谱环形成差异化，三档全可用 |
| **P2（次优先）** | **E23 歌词点阵** | 已有完整逐字歌词基础设施（`WordTimestamp` / `LrcParser` / `LyricsManager`），工期从 2.5 天降为 1.5–2 天；视觉冲击力强、用户感知价值最高；与现有歌词系统深度联动 |
| **P3（第三优先）** | **E22 极光** | 氛围感强、适合慢歌/纯音乐，填补现有效果中"柔和氛围类"的空白，但中高两档可用，受众面略窄 |

> 优先级调整理由：E23 歌词点阵因复用逐字歌词基础设施，工期与难度显著下降，而用户感知价值最高（歌词是音乐类应用核心功能之一），因此从 P3 提升到 P2。
> 如果三个都做，总工期约 **3.5–4 天**（棱镜 0.5 + 歌词点阵 1.5 + 极光 1.5），建议分两期：v2.8 做棱镜 + 歌词点阵（2 天），v2.9 做极光（1.5 天）。

---

#### 4.22.5 E24 `ECG_WAVE` 心跳 · 心电图式滚动频谱 ★（2026-09-13 新增，已落地）

**概述**：经典 CRT 绿线心电图监护仪风格——一条连续折线从屏幕最右侧生成扫描点，已绘制的波形冻结并整体向左匀速平移，最左端超出屏幕被裁掉。**只取鼓点**：折线不由宽频时域波形 `AudioFrame.waveform` 驱动（会把人声/背景乐器一并画进来，导致折线过密），仅在鼓点命中时注入一个完整的 **P-QRS-T 心搏复合波**，相邻鼓点之间贴中心基线走平。基线固定在屏幕垂直中线，心搏波相对基线上下都有振幅（P/R 波向上，Q/S 波下探）；幅度取 `frame.bassRaw` 归一化，鼓点越重跳得越高。峰顶不绘制任何圆点。

| 维度 | 评估 |
|------|------|
| 可行性 | ✅ 高（复用现有 AudioFrame 低频/节拍通道，无新依赖） |
| 技术复用 | 环形历史缓冲（零 arraycopy）+ `BlendMode.Plus` 辉光 + 列虚拟时间基准匀速滚动 |
| 映射 | `beat` / `pulse` 上升沿 → 触发心搏波；`bassRaw`（除以自身慢速均值）→ 心搏幅度；无鼓点 → 贴中心基线 |
| 配色 | 复古示波器：固定 CRT 绿 `0xFF33FF7A` + 暗绿栅格，不取封面色 |
| 风险 | 无硬风险；`onEnter` 时 canvasSize 可能为 0，历史缓冲在 draw 首帧按尺寸懒分配；无鼓点（纯音乐/弱节奏）时画面为平直线 |
| 预估成本 | 约 1–2ms（2 层 drawPath + 栅格），三档画质全可用 |
| 档位 | BASIC（三档全开） |
| 预计工期 | 0.5 天 |

**设计要点**：
- 环形缓冲 `history: FloatArray(cols)`，按 `(rightPtr+1+i)%cols` 取数绘制（最旧→最新 = 左→右），零 arraycopy
- 心搏波形 `heartbeatAt(ageSec)`：P-QRS-T 分段线性查表（P +0.13 → Q −0.12 → **R 主峰 +1.0** → S −0.30 → T 圆峰 +0.24 → 回基线），跨度约 0.50s，比单根尖刺更像真实心电描记；其余列恒为 0（中心基线）
- **鼓点判定双通道**：`frame.beat`（官方标志，仅 1 帧为 true）**或** `frame.pulse` 上升沿（> 0.18）。因为节拍检测跑 50Hz 而渲染数据只 30fps 更新（`EMIT_INTERVAL_MS=33`），30Hz 屏幕上单帧标志会被整拍跳过 → 用跨帧存活的 pulse 包络补回，解决"有鼓点却不出波形"
- **幅度必须用 `bassRaw`**：`bass` 经 `SpectrumRepository.boost()` 峰值跟随归一化，鼓点瞬间恒为 1.0，会让每次鼓点长得一模一样；`bassRaw / bassAvg`（0.8s EMA）映射到 `0.45..1` 保留强弱差异
- **丢拍控密度**：心搏跨度 0.50s + 最短渲染间隔 `MIN_GAP_MS = 800ms`（≈75 BPM 上限），该窗口内检出的连续鼓点直接丢弃，保证每个心搏完整、彼此不粘连
- 主峰高度 `AMP_FRACTION = 0.26`（占半屏比例；原 0.40 → 压制整体高度）
- 滚动/展开用**列虚拟时间** `colMs`（每列 +`1000/speed`）推进 + 小数累积 `colAccum` 防漂移，与真实帧率解耦；cols≈屏宽/2，默认 90 列/秒（可调）；`colMs` 超 1e6 整体回绕，防 Float 精度失真
- `draw` 内零分配：history/Path 均为成员，仅尺寸变化时 realloc；无任何逐点圆心/圆点绘制
...
| **v6.1** | 2026-09-12 | 新增 §4.22 候选效果可行性分析（棱镜 / 极光 / 歌词点阵） |

...
import com.nasmusic.tv.visualizer.VisualizerMath.nextRandom
import com.nasmusic.tv.visualizer.VisualizerMath.nextRandomSigned
...
class ParticlePool(val capacity: Int) {
    ...
    fun spawnBurst(cx: Float, cy: Float, n: Int, maxSpeed: Float, hueBase: Float, hueSpan: Float) {
        for (i in 0 until n) {
            val a = nextRandom() * 6.2831853f
            val sp = maxSpeed * (0.35f + nextRandom() * 0.65f)
            spawnRadial(cx, cy, a, sp, 1f, hueBase + nextRandomSigned() * hueSpan * 0.5f)
        }
    }
    ...
    fun updateAttract(targets: FloatArray, accel: Float, jitter: Float) {
        ...
        while (i < count) {
            ...
            data[o + VX] += (tx - data[o + X]) * accel + nextRandomSigned() * jitter
            data[o + VY] += (ty - data[o + Y]) * accel + nextRandomSigned() * jitter
            data[o + VX] *= 0.86f
            data[o + VY] *= 0.86f
            data[o + X] += data[o + VX]
            data[o + Y] += data[o + VY]
